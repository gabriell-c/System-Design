  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      const highlights = buildDiffHighlights({
        added_node_ids: result.added_nodes?.map((n: any) => n.id) ?? [],
        removed_node_ids: result.removed_nodes?.map((n: any) => n.id) ?? [],
        changed_node_ids: result.changed_nodes?.map((n: any) => n.id) ?? [],
        added_edge_ids: result.added_edges?.map((e: any) => e.id) ?? [],
        removed_edge_ids: result.removed_edges?.map((e: any) => e.id) ?? [],
      });
      ...
      {diffResult && (
        <div className="space-y-1 rounded-lg border border-white/10 bg-black/20 p-3">
          ...
          <div className="space-y-1 text-[10px] text-slate-400">
            <p><span className="text-emerald-400">●</span> {diffResult.added_nodes?.length ?? 0} nós adicionados</p>
            <p><span className="text-rose-400">●</span> {diffResult.removed_nodes?.length ?? 0} nós removidos</p>
            <p><span className="text-amber-400">●</span> {diffResult.changed_nodes?.length ?? 0} nós modificados</p>
            <p><span className="text-emerald-400">→</span> {diffResult.added_edges?.length ?? 0} arestas adicionadas</p>
            <p><span className="text-rose-400">→</span> {diffResult.removed_edges?.length ?? 0} arestas removidas</p>
          </div>
        </div>
      )}
    </div>
