...
export default function Inspector({ hideAnalysis = false }: { hideAnalysis?: boolean }) {
  ...
  return (
    ...
    >
            ...
            return (
              <Tooltip key={item.id} content={item.hint ?? item.label} side="bottom">
                <button
                  ...
                  className={`flex min-h-6 w-full items-center justify-center gap-2 rounded-lg px-2 py-2 text-[12px] font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] ${
                    active
        ...
        <div className="shrink-0 max-h-[38%] overflow-y-auto border-b border-[var(--border)] px-2 py-2">
          <div className="relative mb-2 px-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-[var(--border-strong)] bg-[var(--surface-2)] py-2 pr-3 pl-8 text-sm text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] placeholder:text-[var(--muted-fg)] focus:border-[var(--accent)]"
            />
          </div>
              ...
              return (
                <Tooltip key={item.id} content={item.hint ?? item.label} side="left" className="w-full">
                  <button
                    ...
                    className={`flex w-full items-center gap-1.5 rounded-lg px-2 py-2 text-left text-[12px] font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] ${
                      active
                        ? "bg-[var(--accent-muted)] text-indigo-200"
                        : "text-[var(--muted)] hover:bg-white/5 hover:text-[var(--foreground)]"
                    }`}
      ...
      {HINTS[tab] && (
        <div className="prose-measure shrink-0 border-b border-[var(--border)] bg-black/20 px-3 py-2 text-[12px] leading-relaxed text-[var(--muted)]">
          {HINTS[tab]}
        </div>
      )}
