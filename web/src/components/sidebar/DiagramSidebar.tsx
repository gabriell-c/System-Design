      ...
      <div className="flex-1 overflow-y-auto py-1">
        {projects.length === 0 && !showCreate ? (
          ...
          projects.map((project) => (
            <div
              key={project.id}
              className={`group ${activeProjectId === project.id ? "bg-zinc-800/80" : ""}`}
            >
              <div
                className={`flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-zinc-800 transition-colors ${
                  activeProjectId === project.id ? "border-l-2 border-blue-500" : ""
                }`}
              ...
              {activeProjectId === project.id && project.diagrams?.length ? (
                <ul className="border-t border-zinc-800/80 pb-1">
                  {project.diagrams.map((d) => (
                    <li key={d.id}>
                      <button
                        type="button"
                        className={`w-full px-4 py-1.5 text-left text-[11px] ${
                          graphId === d.id ? "text-cyan-300 bg-cyan-500/10" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/60"
                        }`}
                        onClick={() => loadGraph(d)}
                      >
