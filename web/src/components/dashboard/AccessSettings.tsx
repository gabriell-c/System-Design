  ...
  return (
    <div className="space-y-3">
      <div className="flex items-start gap-2">
        <Users className="mt-0.5 h-4 w-4 shrink-0 text-[var(--muted-fg)]" />
        <div>
          <p className="text-xs font-medium text-slate-300">Acesso por e-mail</p>
          <p className="mt-0.5 text-sm text-[var(--muted)]">
            <span className="inline-flex items-center gap-2 text-[var(--muted-fg)]">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Mail className="pointer-events-none absolute top-1/2 left-2.5 h-3.5 w-3.5 -translate-y-1/2 text-[var(--muted)]" />
          <input
            type="email"
            value={email}
            ...
            className="w-full rounded-lg border border-[var(--border)] bg-black/30 py-2 pr-3 pl-8 text-sm text-slate-100 placeholder:text-[var(--muted-fg)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)]"
          />
        </div>
        <select
          value={role}
          onChange={(e) => setRole(e.target.value as ProjectAccessRole)}
          className="rounded-lg border border-[var(--border)] bg-black/30 px-2 py-2 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
        ...
        <button
          type="button"
          onClick={addEntry}
          className="inline-flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2 text-sm text-slate-200 hover:bg-white/10"
        >
      ...
      {value.length > 0 && (
        <ul className="overflow-hidden rounded-lg border border-[var(--border)] divide-y divide-white/5">
          {value.map((entry) => (
            <li
              key={entry.email}
            ...
            >
              ...
              <button
                ...
                className="rounded p-2 text-[var(--muted)] hover:bg-rose-500/10 hover:text-rose-300"
      ...
      {value.length === 0 && (
        <PanelEmpty
          ...
          description="Adicione e-mails acima para compartilhar o projeto."
        />
      )}
    </div>
  );
