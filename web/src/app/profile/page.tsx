...
export default function ProfilePage() {
  const { user, fetchProfile, updateProfile, logout, isLoading } = useAuthStore();
    ...
    <div className="min-h-screen bg-[#070b10] p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-8">
          ...
          {msg && (
            <div className={`mb-6 p-3 rounded-lg text-sm ${msg.includes("Failed") ? "bg-red-500/10 border border-red-500/20 text-red-400" : "bg-emerald-500/10 border border-emerald-500/20 text-emerald-400"}`}>
              {msg}
            </div>
          )}
            ...
            <div className="border-t border-slate-800 pt-6">
              ...
              <div className="flex items-center gap-4 mb-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.auto_save_enabled}
              ...
              {form.auto_save_enabled && (
                <div>
                  ...
                  <select value={form.auto_save_interval_minutes}
                    onChange={(e) => update("auto_save_interval_minutes", parseInt(e.target.value))}
                    className="w-full px-4 py-3 rounded-xl bg-slate-800/50 border border-slate-700 text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
