...
class TestGetUser:
    def test_get_user_by_id(self, client):
        ...
        list_resp = client.get("/api/v1/users/", headers=_auth_header(token))
        users = list_resp.json()
        bob = next(u for u in users if u["username"] == "BOB")
        resp = client.get(f"/api/v1/users/{bob['id']}", headers=_auth_header(token))
        assert resp.status_code == 200
        assert resp.json()["username"] == "BOB"

    def test_get_user_not_found(self, client):
        token = _login_senior(client)
        resp = client.get("/api/v1/users/9999", headers=_auth_header(token))
...
class TestUpdateUser:
    ...
    def test_update_user_not_found(self, client):
        token = _login_senior(client)
        resp = client.put(
            "/api/v1/users/9999", json={"email": "x@test.com"},
            headers=_auth_header(token)
        )
        assert resp.status_code == 404
    ...
    def test_update_user_requires_senior(self, client):
        ...
        token = login_resp.json()["access_token"]
        resp = client.put("/api/v1/users/1", json={"email": "x@test.com"},
                          headers=_auth_header(token))
        assert resp.status_code == 403
...
class TestDeleteUser:
    def test_delete_user_success(self, client):
        ...
        resp = client.delete(f"/api/v1/users/{grace['id']}", headers=_auth_header(token))
        assert resp.status_code == 200
    ...
    def test_delete_user_cannot_delete_self(self, client):
        ...
        resp = client.delete(f"/api/v1/users/{senior['id']}", headers=_auth_header(token))
        assert resp.status_code == 400
        assert "cannot delete your own" in resp.json()["detail"].lower()

    def test_delete_user_not_found(self, client):
        token = _login_senior(client)
        resp = client.delete("/api/v1/users/9999", headers=_auth_header(token))
        assert resp.status_code == 404
    ...
    def test_delete_user_then_gone(self, client):
        ...
        client.delete(f"/api/v1/users/{hal['id']}", headers=_auth_header(token))
        resp = client.get(f"/api/v1/users/{hal['id']}", headers=_auth_header(token))
        assert resp.status_code == 404
